...
Ltac no_rule_applies :=
  ...
  | [ Hc : Comp _ (EApp _ _) |- _ ] => inversion Hc; subst; discriminate
...
Definition and_xx : @expr model_sorts := EApp (EApp (EPrimOp PAnd) x_var) x_var.
Definition over_applied_program : @expr model_sorts := EApp apply_to_true and_xx.
Definition over_env : @environment model_sorts := extend_env · "f" · and_xx.

Lemma over_applied_program_saturated : saturated over_applied_program.
Proof. repeat split. Qed.

Lemma x_var_value : forall g Φ v, eval (Fin (S g)) Φ · x_var v -> v = x_var.
Proof.
  intros g Φ v H. unfold x_var in H. simpl in H.
  inversion H; subst; try no_rule_applies; try reflexivity.
  simpl in *. discriminate.
Qed.

Lemma and_xx_value : forall g Φ v, eval (Fin (S (S g))) Φ · and_xx v -> v = and_xx.
Proof.
  intros g Φ v H. unfold and_xx in H. simpl in H.
  inversion H; subst; try no_rule_applies.
  match goal with
  | [ Hu : unspool_app _ _ = (EPrimOp _, _), HF : Forall2 _ _ _ |- _ ] =>
      simpl in Hu; injection Hu as <- <-;
      inversion HF as [| a1 v1 l1 l1' H1 HF1]; subst;
      inversion HF1 as [| a2 v2 l2 l2' H2 HF2]; subst;
      inversion HF2; subst
  end.
  rewrite (x_var_value g Φ v1 H1), (x_var_value g Φ v2 H2). reflexivity.
Qed.

Lemma over_applied_prim_app_stuck : forall k Φ Γ v,
  ~ eval (Fin (S k)) Φ Γ (EApp and_xx tt) v.
Proof.
  intros k Φ Γ v H. unfold and_xx in H. simpl in H.
  inversion H; subst; no_rule_applies.
Qed.

Lemma over_var_f_value : forall g Φ v,
  eval (Fin (S (S (S g)))) Φ over_env (EVar "f") v -> v = and_xx.
Proof.
  intros g Φ v H. simpl in H.
  inversion H; subst; try no_rule_applies.
  simpl in *. match goal with [ Hl : Some _ = Some _ |- _ ] => injection Hl as <- <- end.
  eapply and_xx_value. eassumption.
Qed.

Lemma over_body_no_value : forall g Φ v,
  ~ eval (Fin (S (S (S (S g))))) Φ over_env (EApp (EVar "f") tt) v.
Proof.
  intros g Φ v H. simpl in H.
  inversion H; subst; try no_rule_applies.
  match goal with
  | [ Hf : eval _ _ _ (EVar _) ?ef, Happ : eval _ _ _ (EApp ?ef _) _ |- _ ] =>
      rewrite (over_var_f_value g Φ ef Hf) in Happ;
      exact (over_applied_prim_app_stuck _ _ _ _ Happ)
  end.
Qed.

Theorem over_applied_program_no_value : forall g Φ v,
  ~ eval (Fin (S (S (S (S (S (S g))))))) Φ · over_applied_program v.
Proof.
  intros g Φ v H. unfold over_applied_program, apply_to_true in H. simpl in H.
  inversion H; subst; try no_rule_applies.
  match goal with
  | [ Hl : eval _ _ _ (ELam _ _) ?ef, Happ : eval _ _ _ (EApp ?ef _) _ |- _ ] =>
      simpl in Hl; inversion Hl; subst; try no_rule_applies;
      simpl in Happ; inversion Happ; subst; try no_rule_applies
  end.
  exact (over_body_no_value g Φ v ltac:(eassumption)).
Qed.

Theorem over_applied_program_not_budget_total : forall Φ, ~ budget_total Φ · over_applied_program.
Proof.
  intros Φ [h Hh].
  destruct (Hh (S (S (S (S (S (S h)))))) ltac:(lia)) as [v Hv].
  exact (over_applied_program_no_value h Φ v Hv).
Qed.
